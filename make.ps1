...
<#
.Description
Make file for MoMapper (Mongo Mapper)
#>
...
#>
function lint() {
    poetry run flake8 momapper tests
}
...
#>
function coverage() {
    poetry run coverage run --source momapper -m pytest
...
#>
function docs(){
    if (Test-Path ".\docs\momapper.rst") { Remove-Item ".\docs\momapper.rst" -Force }
    if (Test-Path ".\docs\modules.rst") { Remove-Item ".\docs\modules.rst" -Force }
    poetry run sphinx-apidoc -o docs\ momapper
